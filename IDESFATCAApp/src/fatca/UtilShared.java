...
public class UtilShared {
    ...
    protected static void printNode(Node node) {
		System.out.println("--> printNode() " + node.getNodeName());
		System.out.println("prefix=" + node.getPrefix() + ", baseuri=" + node.getBaseURI() + ", nsuri=" + node.getNamespaceURI() + ", value=" + node.getNodeValue());
		...
		if (node.getNextSibling() != null) {
			System.out.println("--> Sibling of " + node.getNodeName() );
	...
	public static boolean verifySignatureDOM(String signedFile, PublicKey sigkey, boolean useFakeElem) throws Exception  {
		System.out.println("--> verifySignatureDOM(). signedFile=" + signedFile);
		boolean ret = false;
		...
		try {
			...
			System.out.println("isFakeElem=" + isFakeElem);
			if (isFakeElem) {
			...
			System.out.println("Signature core validation " + coreValidity);
		    SignatureValue sigval = signature.getSignatureValue();
		    sigValValidity = sigval.validate(valContext);
		    System.out.println("SignatureValue validation status: " + sigValValidity);
	        ...
	        for (int j=0; iter.hasNext(); j++) {
	        	...
	        	System.out.println("ref.getCalculatedDigestValue()=" + Base64.encode(ref.getCalculatedDigestValue()));
	        	System.out.println("ref.getReferencedDigestValue()=" + Base64.encode(ref.getDigestValue()));
	        	List<?> lt = ref.getTransforms();
	        	Transform tr;
	        	for (int i = 0; i < lt.size(); i++) {
	        		tr = (Transform)lt.get(i);
	        		System.out.println("transform.getAlgorithm()=" + tr.getAlgorithm());
	        	}
	        }
			ret = coreValidity & sigValValidity & refValidFlag;
			System.out.println("signature validated=" + ret);
		} catch (Exception e) {
			e.printStackTrace();
			//throw e; // do not throw
		} finally {
		}
		System.out.println("<-- verifySignatureDOM()");
		return ret;
	}
	
	public static void createBinaryFileFromSignedBase64BinaryFile(String infile, String outfile) throws Exception {
		System.out.println("--> createBinaryFileFromSignedBase64BinaryFile(). infile=" + infile + ", outfile=" + outfile);
		...
		System.out.println("<-- createBinaryFileFromSignedBase64BinaryFile()");
	}
